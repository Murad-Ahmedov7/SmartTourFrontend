...
//   // const [CurrentPassword,setCurrentPassword]=useState(validPassword);

//      const [CurrentPassword,setCurrentPassword]=useState('');


//   const [NewPassword,setNewPassword]=useState('murad12345');
...
//       { CurrentPassword,  NewPassword }, // camelCase olmalıdır
...
export default function EditableField({ label, initialValue, isPassword = false, onSave }) {
  ...
  const handleSave = async () => {
    ...
    if (isPassword && currentPassword && newPassword) {
      try {
        ...
        setCurrentPassword(""); // clear current input
        setNewPassword("");     // clear new input
      } catch (err) {
        if (err.response) {
          console.log("Error status:", err.response.status);
          console.log("Error data:", err.response.data);
  ...
  const maskedPassword = "•".repeat(currentPassword.length || newPassword.length);
    ...
    <div className="profilPage-info-row">
      <div className="profilPage-label-value-wrapper">
        ...
        {isEditing ? (
          <input
            type={isPassword ? (showPassword ? "text" : "password") : "text"}
            value={isPassword ? newPassword : value}
            onChange={e => isPassword ? setNewPassword(e.target.value) : setValue(e.target.value)}
            className="profilPage-edit-input"
            autoFocus
          />
        ) : (
          <span className="profilPage-info-value">
            {isPassword ? (showPassword ? currentPassword || newPassword : maskedPassword) : value || "Not provided"}
          </span>
        )}
      </div>
      ...
      {isPassword && (
        <button onClick={toggleShowPassword} style={{ border: "none" }}>
          {showPassword ? <FaRegEyeSlash /> : <FaRegEye />} {showPassword ? "Hide" : "Show"}
        </button>
      )}
